...
document.getElementById('backButton').addEventListener('click', function() {
    ...
    // Affichage dynamique des diagrammes
    function initDiagramTabs() {
        const diagramTabs = document.createElement('div');
        diagramTabs.className = 'diagram-tabs';
        diagramTabs.innerHTML = `
            <button class="diagram-tab active" data-diagram="architecture">Architecture</button>
            <button class="diagram-tab" data-diagram="sequence">Séquence</button>
            <button class="diagram-tab" data-diagram="database">Base de données</button>
        `;
        
        const diagramContainer = document.querySelector('.architecture-diagram');
        diagramContainer.prepend(diagramTabs);
        
        // Gestion des clics
        diagramTabs.querySelectorAll('.diagram-tab').forEach(tab => {
            tab.addEventListener('click', function() {
                // Implémentation à compléter avec la logique de changement d'image
            });
        });
    }

    // Initialisation au chargement
    document.addEventListener('DOMContentLoaded', initDiagramTabs);